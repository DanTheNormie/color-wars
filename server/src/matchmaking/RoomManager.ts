...
export class RoomManager {
  ...
  static async createPrivateRoom(
    options: PrivateRoomCreation = {},
  ): Promise<{ joinCode: string; reservation: SeatReservationResult }> {
  ...
  static async joinRoomById(
    ...
    if (!record) {
      ...
      if (
        listing.clients >= listing.maxClients ||
        metadata.phase === "active" ||
        metadata.phase === "finished"
      ) {
        logger.info("Joining as spectator", {
          roomId,
    ...
    if (phase === "active" || room.clients.length >= room.maxClients) {
      logger.info("Room is active or full, joining as spectator", {
        roomId,
        phase,
